...
function transformToLis(obj){
  // Solution code here...
  let givenobj = Object.entries(obj);
  let tohtmlStyle = [];
 for(let i = 0; i < givenobj.length; i++){
  for(let j = 0; j < givenobj[i].length-1; j ++){
    tohtmlStyle.push(`<li>${givenobj[i][j]}: ${givenobj[i][j+1]}</li>`)
  }
 }
 return tohtmlStyle;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let returnArray = input.map(item => item.filter(element => element%5 === 0))
  returnArray = returnArray.map(item => item.filter(element => typeof(element) === 'number'))
  return returnArray.map(item => item.map(element => Math.pow(2, element)))
