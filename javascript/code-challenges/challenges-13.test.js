...
// Solution code here...
 ...
 let indexWord = -1 ;
 arr.forEach(item => {
  if (stringWord.length < item.length){
    stringWord = item;
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  const ReturnStandardPhone = [];
  arr.forEach(item => {

    ReturnStandardPhone.push(item.substring(1,4)+item.substring(6,9)+item.substring(10,14))
    
  })
  return ReturnStandardPhone;
};
