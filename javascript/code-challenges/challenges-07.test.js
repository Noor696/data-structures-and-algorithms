...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let sortArr = starWarsArr.sort((a,b) =>{
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= result.length; i++) {
      shorterArr.push(result.slice(i).join(''))
  }
  return shorterArr;
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let test01=recipe.ingredients[i]
    let test02=test01.slice(test01.indexOf(" ")+1)
    let test03=test02.slice(test02.indexOf(" ")+1)
    result.push(test03)
  }
  return result;
};
