...
const maxInArray = (arr) => {
  // Solution code here...
  return (arr.reduce((previousV, currentV)=>{
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const ChildrenArr = arr.filter(item => Object.values(item).length > 3);
	if(ChildrenArr.filter(item => item.name === character).length > 0){
		return true
	} else {
		return false
	}
